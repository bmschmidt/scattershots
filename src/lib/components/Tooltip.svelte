<script lang="ts">
  ...
  async function short_text(title) {
    ...
    const r = await fetch(`//en.wikipedia.org/w/api.php?action=query&origin=*&prop=extracts&format=json&exintro=&titles=${encoded_title}`)
  ...
  const index_fields = [
    {
      label: "title", f: "title", filter: true
    },
    {
      label: "topic_top_20", f: "topic_top_20", filter: true
    },
    {
      label: "id", f: "id", filter: false
    },
    {
      f: "topic_top_50", label: "topic_top_50", filter: true
    },
    {
      f: "topic_top_100", label: "topic_top_100", filter: true
    },
  ]
