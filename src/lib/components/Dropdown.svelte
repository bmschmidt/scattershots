...
<div>
  ...
  <div class="my-2">
  {#if proposed !== ''}
  <select name="cars" id="cars" bind:value={select_input} 
    on:change={accept_input}>
  ...
  <div class="flex flex-row flex-wrap">
    {#each selections as selection}
      <div class="flex flex-row rounded-full border m-2 bg-gray-300">
        ...
        <div on:click={() => drop(selection)} class="hover:bg-gray-900 hover:text-white px-3 mx-2">x</div>
        </div>
      {/each}
    </div>
  </div>
